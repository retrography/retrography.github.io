...
Most JSON files I need to convert are arrays containing elements (i.e. documents) with similar or identical format. Their elements are not very complex, although sometimes they may contain nested structures. All I expect from a convertor is to (1) detect the schema automatically and (2) flatten the structure and adapt the schema. Flattening involves both extracting embedded hashes and converting embedded arrays into strings. What I am particularly wary of is creating outline files for JSON extraction or having to detect the schema manually in order to name the required fields. 
